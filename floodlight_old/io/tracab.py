...
from floodlight_old.core.code import Code
from floodlight_old.core.pitch import Pitch
from floodlight_old.core.xy import XY
from floodlight_old.core.teamsheet import Teamsheet
from floodlight_old.io.utils import get_and_convert
...
def _read_dat_single_line(
    ...
    for t in targets:
        ...
        x, y, = map(lambda x: float(x), player_data[3:5])
        if team == 1:
            team = "Home"
        elif team == 0:
            team = "Away"
...
def _frame_in_period(
    ...
    periods: Dict[str, Tuple[int, int]]
        ...
        :meth:`floodlight_old.io.tracab._read_metadata`.
...
def read_position_data_dat(
    ...
    data_objects: Tuple[Dict[str, Dict[str, XY]], Dict[str, Code], Dict[str, Code], \
     Dict[str, Teamsheet], Pitch]
        Tuple of (nested) floodlight_old core objects with shape (xy_objects,
    ...
    with open(filepath_dat, "r") as f:
        while True:
            ...
            print(frame_abs, positions, ball)
